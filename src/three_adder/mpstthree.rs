...
pub fn adder_a(s: EndpointA) -> Result<()> {
    ...
    let (Add(y), s) = recv_mpst_a_to_b(s)?;
    let s = send_mpst_a_to_c(Add(y), s);
    let (Sum(z), s) = recv_mpst_a_to_c(s)?;
    assert_eq!(z, 5);
    close_mpst(s)?;
    Ok(())
}
...
pub fn adder_b(s: EndpointB) -> Result<()> {
    let (Add(y), s) = recv_mpst_b_to_a(s)?;
    let x = 3;
    let s = send_mpst_b_to_a(Add(x), s);
    let s = send_mpst_b_to_c(Add(y), s);
    let (Sum(z), s) = recv_mpst_b_to_c(s)?;
    assert_eq!(z, 5);
    close_mpst(s)?;
    Ok(())
}
...
pub fn adder_c(s: EndpointC) -> Result<()> {
    let (Add(x), s) = recv_mpst_c_to_a(s)?;
    let (Add(y), s) = recv_mpst_c_to_b(s)?;
    let z = x + y;
